...
db = pymysql.connect(host='8.129.186.89',
                     ...
                     charset='utf8')
cur = db.cursor()
ex = xlrd.open_workbook("半成品.xlsx")
...
for row_num in range(1, rows):
    ...
    warehouse_id, materiel_id, materiel_stock = data
    ...
    sql = """insert into
                warehouse_detail(warehouse_id, materiel_id, materiel_stock)
            value
            ('%s','%s','%s')""" % (warehouse_id, materiel_id, materiel_stock)
    try:
        # 执行sql语句
        cur.execute(sql)
        # 提交数据到数据库
        db.commit()
        print(f"成功了", materiel_id)

    except Exception as e:
        # 对修改失败的数据进行撤销,表示回滚
        db.rollback()
        print(f"失败了", materiel_id)
        print(e)
